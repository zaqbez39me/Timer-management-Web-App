...
function refresh(timers) {
    for (let i = 0; i < timers.length; i++) {
        ...
        updateCountdown.call(forms[0], new PointerEvent("click"), true)
        if (!obj["active"]) {
            console.log(forms[1].querySelector(".timer-button"))
            pause.call(forms[1].querySelector(".timer-button"), new PointerEvent("click"), true)
        }
        console.dir(timers)
...
async function updateDom(f0rm, timerElements, inputValue, timerTitle, refreshTime = false) {
    let nowTime = 0
...
function updateCountdown(e, refreshTime = false) {
    ...
    isPaused = false
    updateDom(f0rm, timerElements, inputValue * 1000, inputTitle, refreshTime)
}
...
async function pause(e, refreshTime = false) {
    f1rm = this.closest(".timer")
